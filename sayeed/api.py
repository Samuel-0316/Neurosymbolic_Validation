...
@app.route('/validate_triple', methods=['POST'])
def api_validate_triple():
    data = request.get_json()
    subject = data.get('subject')
    predicate = data.get('predicate')
    # Accept both 'obj' and 'object' for compatibility
    obj = data.get('obj') or data.get('object')
    if not subject or not predicate or not obj:
        return jsonify({"error": "Missing subject, predicate, or obj"}), 400
    result = validate_triple(subject, predicate, obj)
